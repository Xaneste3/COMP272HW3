...
class LUC_AVLTree {
    ...
    private Node deleteElement(int value, Node node) {
        ...
        //If node passed as a parameter is null, return null
        if (node == null) {
            return null;
        }
        ...
        //If a value is found and no children then return null to delete
        else {
            if (node.leftChild == null && node.rightChild == null) {
                return null;
            }

            //If there is one subtree
        ...
        //Now that the tree has been manipulated the height needs to be recalculated
        //The node balance factor needs to be recalculated as well
        node.height = getMaxHeight(getHeight(node.rightChild), getHeight(node.leftChild)) +1;
        int balanceFactor = getBalanceFactor(node);

        //Any rotations whether they are RR, LL, RL, LR will occur here

        //This is for LL or LR
        if (balanceFactor > 1) {
